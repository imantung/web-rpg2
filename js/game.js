...
Game.create = function() {
  ...
  // Display map
  Game.map = Game.scene.make.tilemap({
    key: 'map'
  });
  ...
  cursor = new Cursor(TILE_SIZE);
  cursor.create(this)
  // Game.marker = this.add.graphics();
  // Game.marker.lineStyle(3, 0xff0000, 1);
  ...
  for (var i = tileset.firstgid - 1; i < tiles.total; i++) { // firstgid and total are fields from Tiled that indicate the range of IDs that the tiles can take in that tileset
    ...
    if (properties[i].cost) {
      Game.finder.setTileCost(i + 1, properties[i].cost); // If there is a cost attached to the tile, let's register it
    }
  }
...
Game.update = function() {
  ...
  cursor.setX(Game.map.tileToWorldX(pointerTileX))
  cursor.setY(Game.map.tileToWorldY(pointerTileY))
  cursor.setVisible(!Game.checkCollision(pointerTileX, pointerTileY))
};
